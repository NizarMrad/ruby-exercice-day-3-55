...
puts array_Twitter.length

#Quelle est le handle le plus court de cette liste ?

puts array_Twitter.min_by{|obj| obj.size}

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

puts array_Twitter.select{|word|word.length == 6}

#Combien commencent par une majuscule (première lettre juste après le @) ?

puts array_Twitter.count {|e| e =~ /^@[A-Z]/}

#Trie la liste de handle par ordre alphabétique.

puts array_Twitter.sort_by!{ |e| e.downcase }

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)

# puts array_Twitter.sort_by(&:length) ou 
puts array_Twitter.sort_by { |obj| obj.size }


#Quelle est la position dans l'array de la personne @epenser ?

puts array_Twitter.find_index("@epenser")
...
handle_length = []
array_Twitter.each {|handle| handle_length << handle.length }

hash_twitter = Hash[array_Twitter.zip(handle_length)] 

i = 1
while i < 34
    print "nombre de string avec #{i-1} caractères: "
    hash_twitter_char = Hash[hash_twitter.select { |k, v| v == i}]
    puts hash_twitter_char.count
    i += 1
end
